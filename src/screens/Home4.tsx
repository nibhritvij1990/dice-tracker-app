...
const Home4: React.FC = () => {
  ...
  function onHeaderSelectPlayers(n: number) {
    ...
    try { window.dispatchEvent(new CustomEvent('home4:setPlayers', { detail: n })); } catch {}
  }
  function onHeaderGenerate() {
    try { window.dispatchEvent(new CustomEvent('home4:generate')); } catch {}
  }
  ...
  function CatanSetup({ desktop }: { desktop: boolean }) {
    ...
    useEffect(() => {
      ...
      window.addEventListener('home4:setPlayers', handleSetPlayers as EventListener);
      window.addEventListener('home4:generate', handleGenerate as EventListener);
      return () => {
        window.removeEventListener('home4:setPlayers', handleSetPlayers as EventListener);
        window.removeEventListener('home4:generate', handleGenerate as EventListener);
      };
    }, []);
    ...
    return (
      <div className="relative w-full h-full select-none" id="catan-map-container" ref={wrapperRef}>
        <div
          ...
          className={`absolute left-1/2 top-1/2 ${isDesktop ? '-translate-x-[57%]' : '-translate-x-1/2'} ${isDesktop ? '-translate-y-[57%]' : '-translate-y-1/2'}`}
    ...
    <div className="app-safe overflow-hidden bg-gradient-to-br from-[#2E1371] to-[#130B2B] text-white relative">
        ...
        {isDesktop && (
          <div className="relative" style={{ height: '100dvh', paddingTop: 'var(--safe-top)', paddingBottom: 'var(--safe-bottom)' }}>
              ...
              <aside className="sticky top-0 h-screen overflow-y-auto shrink-0 w-[240px] px-4 py-6 flex flex-col justify-between z-[10]">
                <div>
                  ...
                  <nav className="flex flex-col gap-2 text-sm mt-2" id="main-nav">
                    <Link to="/home2" className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-[#f9fafb] text-[#2E1371]">Map Generator</Link>
                  ...
                  <header className="p-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="mt-2 text-gray-500">Generate balanced Catan boards for 4/5/6 players.</p>
                      </div>
                      {/* Right: Controls */}
                      <div className="w-[200px] shrink-0 rounded-xl border border-gray-200 bg-white shadow-lg p-4 text-gray-900 absolute right-[1.5rem] top-[116px]">
            ...
            <button onClick={onHeaderGenerate} className="w-full h-10 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm">Generate Map</button>
          </div>
                    </div>
                  </header>
