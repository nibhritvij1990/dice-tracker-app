    ...
    if (desktop) {
      return (
        <div className="flex items-start gap-6 w-full h-full">
          ...
          <div className={`relative flex-1 ${isDesktop ? 'min-h-[calc(100vh-160px)]' : 'min-h-[60vh]'}`}>
          ...
          {/* Right: Controls fixed width 200px */}
          <div className="w-[200px] shrink-0 rounded-xl border border-gray-200 bg-white shadow-lg p-4 text-gray-900">
              ...
              <div className="inline-flex items-center gap-2">
                {[4,5,6].map(n => (
                  <button key={n} onClick={() => { setPlayers(n); try { localStorage.setItem('catan_players', String(n)); } catch {} }}
                    className={`h-9 w-9 rounded-lg border text-sm ${players===n ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-300 bg-white text-gray-900 hover:bg-gray-100'}`}>
                    {n}
                  </button>
                ))}
              </div>
            </div>
            <button onClick={onGenerate} className="w-full h-10 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm">Generate Map</button>
          </div>
        </div>
      );
    }
    ...
    useEffect(() => {
      ...
      const adjust = () => {
        ...
        if (boardContainerRef.current) {
          if (isDesktop) {
            (boardContainerRef.current as HTMLDivElement).style.zoom = String(0.95 * desired);
        } else {
            ...
            <div className="relative w-full h-full flex">
              ...
              <main className="flex-1 p-6 m-2 overflow-y-auto min-h-0 relative" style={{ borderRadius: '16px', backgroundColor: 'rgb(249 250 251 / var(--tw-bg-opacity, 1))', zIndex: 1 }}>
                ...
                <div className="w-full">
                  ...
                  <div className={isDesktop ? 'relative overflow-hidden' : 'relative overflow-hidden min-h-[calc(100vh-160px)] rounded-2xl border border-gray-200 bg-white'}>
