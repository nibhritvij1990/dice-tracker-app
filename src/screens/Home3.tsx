...
const Home3: React.FC = () => {
  ...
  function onHeaderSelectPlayers(n: number) {
    ...
    try { window.dispatchEvent(new CustomEvent('home3:setPlayers', { detail: n })); } catch {}
  }
  function onHeaderGenerate() {
    try { window.dispatchEvent(new CustomEvent('home3:generate')); } catch {}
  }
  ...
  function CatanSetup({ desktop }: { desktop: boolean }) {
    ...
    useEffect(() => {
      ...
      window.addEventListener('home3:setPlayers', handleSetPlayers as EventListener);
      window.addEventListener('home3:generate', handleGenerate as EventListener);
      return () => {
        window.removeEventListener('home3:setPlayers', handleSetPlayers as EventListener);
        window.removeEventListener('home3:generate', handleGenerate as EventListener);
      };
    }, []);
        ...
        {isDesktop && (
          <div className="relative" style={{ height: '100dvh', paddingTop: 'var(--safe-top)', paddingBottom: 'var(--safe-bottom)' }}>
            ...
            <div className="relative w-full h-full flex">
              ...
              <main className="flex-1 p-0 m-2 overflow-y-auto min-h-0 relative" style={{ borderRadius: '16px', backgroundColor: 'rgb(249 250 251 / var(--tw-bg-opacity, 1))', zIndex: 1 }}>
